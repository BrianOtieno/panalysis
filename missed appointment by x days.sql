...
max(p.appointment_date + INTERVAL 7 DAY) as "End of Grace Window",
...
CONCAT_WS(" ", DATEDIFF(CURDATE(), max(p.appointment_date + INTERVAL 7 DAY)), 
   "day(s) after appointment grace window of 7 days") Intepratation,
...
WHERE 
(p.appointment_date + INTERVAL 8 DAY) <= NOW()
-- AND 
-- i.identifier_type = 4
